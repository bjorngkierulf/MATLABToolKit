function SuperState = breakCaseSuperHeat(Prop1,Value1,Prop2,Value2,Table,Critical,debug)
...
if strcmp(Prop2,'P') %works
    ...
    switch Prop1
        case 'T'
            ...
            if debug, fprintf('\nFirst Isobar value=%f',firstIsoBarValue); end
        ...
        case 's' %same as 'v'?
            ...
            firstIsoBarValue = PVec(firstIsoBarInd); %resets this
            if debug, fprintf('\nFirst Isobar value=%f=%f',firstIsoBarValue); end

        otherwise
            fprintf("\nmust be T, v, or s")
    end
    secondIsoBarValue = PVec(secondIsoBarInd);
    if debug, fprintf('\nSecond Isobar value=%f',secondIsoBarValue); end
    ...
    elseif strcmp(Prop1,'s')
        ...
        if debug, fprintf("\nh, u, properties not supported for this type of interpolation"); end
    end
...
elseif strcmp(Prop1,'T') %P is not an input, so we have T, v and / or s.
    ...
    if strcmp(Prop2,'v')
        ...
        if debug
            a = NewTable.P
            fprintf('\nFirst Isobar Value=%f',firstIsoBarValue)
        end
        ...
        if debug
            firstIsoBarSpecEntropys
            fprintf('\nSpec entropy at T input=%f',firstIsoBarSpecEntropyTInput)
        end
        ...
        if supercritical > 0
            ...
            if debug
                fprintf('\nqP=%f, qv=%f, qS=%f, qU=%f, qH=%f',qPSat,qvSat,qsSat,quSat,qhSat)
            end
        end
        ...
        if debug
            fprintf('\nqP=%f, qS=%f, qU=%f, qH=%f',qP,qS,qU,qH)
        end
    ...
    elseif strcmp(Prop2,'s')
        ...
        if debug
            ...
            fprintf('\nTInput=%f, SInput=%f',TInput,sInput)
        end
        ...
        if supercritical > 0
            ...
            if debug
                fprintf('\nqPSat=%f, qvSat=%f, qsSat=%f, quSat=%f, qhSat=%f',qPSat,qvSat,qsSat,quSat,qhSat)
            end
        end
        ...
        if debug
            fprintf('\nqP=%f, qV=%f, qU=%f, qH=%f',qP,qV,qU,qH)
        end
